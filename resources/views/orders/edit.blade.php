    ...
    <form action="{{ route('orders.update',$order->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <select name="employe_id" id="" name="service"  class="form-control">
                    ...
                    @foreach ($employes->all() as $employe)
                     <option value="{{$employe->id}}" {{$employe->id ==$order->employe_id ? 'selected': '' }}>{{$employe->name}}</option>
                    @endforeach
                </select>
             </div>
        </div>
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                ...
                <input class="form-control"  name="hours" placeholder="number of hours to execute the service" value="{{ $order->hours }}">
            </div>
        </div>
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                <strong>Tax value:</strong>
                <input class="form-control"  name="tax" placeholder="tax for the order" value="{{ $order->tax }}" >
            </div>
        </div>
